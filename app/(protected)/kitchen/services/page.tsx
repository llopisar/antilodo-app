...
export default async function KitchenServicesPage({ searchParams }: Props) {
  ...
  return (
    <div className="space-y-4">
      <ModuleHeader title="Kitchen Services" description="Service windows, status progression, and staffing context." />
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <InlineStat label="Planned" value={String(services.filter((s) => s.status === "PLANNED").length)} />
      </div>

      <FilterForm action="/kitchen/services" query={params.q} status={params.status} statusOptions={["PLANNED", "ACTIVE", "COMPLETED", "CANCELLED"]} />
      ...
      <div className="grid gap-3">
        ...
        {services.map((service) => (
          <ListCard
            ...
            href={`/kitchen/services/${service.id}`}
